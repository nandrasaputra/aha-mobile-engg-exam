...
import com.darkempire78.opencalculator.databinding.CalculatorFragmentBinding
...
class CalculatorFragment : Fragment() {
    ...
    private val viewModel: CalculatorSharedViewModel by activityViewModels()
    ...
    private fun observeData() {
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.arrowClickEvent.collect { event ->
                    val spec = event.peek()
                    if (spec != null && spec.originId == fragmentContainerId) {
                        event.consumeOnce {
                            viewModel.updateArrowData(spec.originId, binding.input.text.toString())
                        }
                    }
                }
            }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.calculatorDataFlow.collect { event ->
                    val spec = event.peek()
                    if (spec != null && spec.originId != fragmentContainerId) {
                        event.consumeOnce {
                            binding.input.setText(binding.input.text.toString() + spec.data)
    ...
    private fun setupView(context: Context) {
        ...
        binding.oneButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.twoButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.threeButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.fourButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.fiveButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.sixButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.sevenButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.eightButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.nineButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.zeroButton.setOnClickAndGainFocus(binding.input) {
            keyDigitPadMappingToDisplay(it)
        }

        binding.pointButton.setOnClickAndGainFocus(binding.input) {
            pointButton(it)
        }

        binding.backspaceButton.setOnClickAndGainFocus(binding.input) {
            backspaceButton(it)
        }

        binding.equalsButton.setOnClickAndGainFocus(binding.input) {
            equalsButton(it)
        }

        binding.addButton.setOnClickAndGainFocus(binding.input) {
            addButton(it)
        }

        binding.subtractButton.setOnClickAndGainFocus(binding.input) {
            subtractButton(it)
        }

        binding.multiplyButton.setOnClickAndGainFocus(binding.input) {
            multiplyButton(it)
        }

        binding.divideButton.setOnClickAndGainFocus(binding.input) {
            divideButton(it)
        }

        binding.divideBy100Button.setOnClickAndGainFocus(binding.input) {
            percent(it)
        }

        binding.parenthesesButton.setOnClickAndGainFocus(binding.input) {
            parenthesesButton(it)
        }

        binding.clearButton.setOnClickAndGainFocus(binding.input) {
            clearButton(it)
        }

        binding.scientistModeSwitchButton?.setOnClickAndGainFocus(binding.input) {
            scientistModeSwitchButton(it)
        }

        binding.squareButton.setOnClickAndGainFocus(binding.input) {
            squareButton(it)
        }

        binding.piButton.setOnClickAndGainFocus(binding.input) {
            piButton(it)
        }

        binding.exponentButton.setOnClickAndGainFocus(binding.input) {
            exponentButton(it)
        }

        binding.factorialButton.setOnClickAndGainFocus(binding.input) {
            factorialButton(it)
        }

        binding.degreeButton.setOnClickAndGainFocus(binding.input) {
            degreeButton(it)
        }

        binding.sineButton.setOnClickAndGainFocus(binding.input) {
            sineButton(it)
        }

        binding.cosineButton.setOnClickAndGainFocus(binding.input) {
            cosineButton(it)
        }

        binding.tangentButton.setOnClickAndGainFocus(binding.input) {
            tangentButton(it)
        }

        binding.invButton.setOnClickAndGainFocus(binding.input) {
            invButton(it)
        }

        binding.eButton.setOnClickAndGainFocus(binding.input) {
            eButton(it)
        }

        binding.naturalLogarithmButton.setOnClickAndGainFocus(binding.input) {
            naturalLogarithmButton(it)
        }

        binding.logarithmButton.setOnClickAndGainFocus(binding.input) {
            logarithmButton(it)
        }

        binding.settingsButton.setOnClickAndGainFocus(binding.input) {
            openAppMenu(it)
        }

        binding.settingsButton.isVisible = fragmentContainerId == R.id.calculatorFragment
        ...
        historyAdapter = HistoryAdapter(mutableListOf()) { value ->
            run {
                ...
                activity?.let {
                    updateDisplay(it.window.decorView, value)
                }
            }
        }
        ...
        // Set values
        val historyList = MyPreferences(context).getHistory()
        historyAdapter.appendHistory(historyList)

        // Disable history if setting enabled
        val historySize = MyPreferences(context).historySize!!.toInt()
        if (historySize == 0) {
        ...
        binding.resultDisplay.setOnLongClickListener {
            when {
                binding.resultDisplay.text.toString() != "" -> {
                    if (MyPreferences(context).longClickToCopyValue) {
                        ...
                        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.S_V2)
                            Toast.makeText(context, R.string.value_copied, Toast.LENGTH_SHORT)
                                .show()
                        true
                    } else {
                        false
    ...
    private fun openAppMenu(view: View) {
        ...
        popup.setOnMenuItemClickListener { item ->
            when (item?.itemId) {
                R.id.app_menu_clear_history_button -> {
                    clearHistory(view.context, item)
                    true
                }
    ...
    fun openAbout(context: Context, menuItem: MenuItem) {
    ...
    fun clearHistory(context: Context, menuItem: MenuItem) {
        // Clear preferences
        MyPreferences(context).saveHistory(context, mutableListOf())
        // Clear drawer
        historyAdapter.clearHistory()
    }

    private fun keyVibration(view: View) {
        if (MyPreferences(view.context).vibrationMode) {
    ...
    private fun setErrorColor(context: Context, errorStatus: Boolean) {
        ...
        if (errorStatus != errorStatusOld) {
            ...
            // Clear error color
                ...
                binding.input.setTextColor(ContextCompat.getColor(context, R.color.text_color))
                binding.resultDisplay.setTextColor(
                    ContextCompat.getColor(
                        context,
    ...
    private fun roundResult(context: Context, result: BigDecimal): BigDecimal {
        val numberPrecision = MyPreferences(context).numberPrecision!!.toInt()
        var newResult = result.setScale(numberPrecision, RoundingMode.HALF_EVEN)
        if (MyPreferences(context).numberIntoScientificNotation && (newResult >= BigDecimal(9999) || newResult <= BigDecimal(
                0.1
            ))
        ) {
    ...
    private fun updateResultDisplay(context: Context) {
        lifecycleScope.launch(Dispatchers.Default) {
            // Reset text color
            setErrorColor(context, false)
            ...
            if (calculation != "") {
                ...
                context?.let {
                    calculationResult =
                        Calculator(MyPreferences(it).numberPrecision!!.toInt()).evaluate(
                            calculationTmp,
                ...
                if (!(division_by_0 || domain_error || syntax_error || is_infinity || require_real_number)) {
                    ...
                    // Round
                    calculationResult = roundResult(context, calculationResult)
                    ...
                    if (!MyPreferences(context).numberIntoScientificNotation || !(calculationResult >= BigDecimal(
                            9999
    ...
    fun squareButton(view: View) {
        if (!isInvButtonClicked) {
            ...
            if (MyPreferences(view.context).addModuloButton) {
                updateDisplay(view, "#")
            } else {
    ...
    fun degreeButton(view: View) {
        keyVibration(view)
        toggleDegreeMode()
        updateResultDisplay(view.context)
    }
                ...
                if (calculation != "") {
                    ...
                    if (!(division_by_0 || domain_error || syntax_error || is_infinity || require_real_number)) {
                        ...
                        if (calculation != formattedResult) {
                            val history = MyPreferences(context).getHistory()
                            ...
                            if (history.isEmpty() || history[history.size - 1].calculation != calculation) {
                                ...
                                MyPreferences(context).saveHistory(context, history)
                                ...
                                withContext(Dispatchers.Main) {
                                    ...
                                    val historySize =
                                        MyPreferences(context).historySize!!.toInt()
                        ...
                        withContext(Dispatchers.Main) {
                            if (syntax_error) {
                                setErrorColor(view.context, true)
                                ...
                                else binding.resultDisplay.text =
                                    getString(R.string.value_too_large)
    ...
    override fun onResume() {
        ...
        context?.apply {
            ...
            val history = MyPreferences(this).getHistory()
            while (historySize > 0 && history.size > historySize) {
                history.removeAt(0)
            }
            MyPreferences(this).saveHistory(this, history)
            ...
            binding.inputHorizontalScrollView.postDelayed(
                Runnable { binding.inputHorizontalScrollView.fullScroll(HorizontalScrollView.FOCUS_RIGHT) },
                100L
            )
        }
    }
...
private fun View.setOnClickAndGainFocus(editText: EditText, action: (View) -> Unit) {
    setOnClickListener {
